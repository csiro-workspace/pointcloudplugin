...
)
{
  ...
  if (elem == NULL) {
    fprintf(stdout,"element_layout_ply: can't find element '%s'\n",elem_name);
    exit (-1);
  }
...
)
{
  ...
  elem = find_element (plyfile, elem_name);
  if (elem == NULL) {
    fprintf(stdout, "ply_describe_property: can't find element '%s'\n",
            elem_name);
    return;
  }
...
)
{
  ...
  elem = find_element (plyfile, elem_name);
  if (elem == NULL) {
    fprintf(stdout,"element_count_ply: can't find element '%s'\n",elem_name);
    exit (-1);
  }
...
void header_complete_ply(PlyFile *plyfile)
{
  ...
  switch (plyfile->file_type) {
    ...
    case PLY_BINARY_LE:
      ...
      break;
    default:
      fprintf (stdout, "ply_header_complete: bad file type = %d\n",
               plyfile->file_type);
      exit (-1);
  }
...
void put_element_setup_ply(PlyFile *plyfile, char *elem_name)
{
  ...
  elem = find_element (plyfile, elem_name);
  if (elem == NULL) {
    fprintf(stdout, "put_element_setup_ply: can't find element '%s'\n", elem_name);
    exit (-1);
  }
...
)
{
  ...
  for (i = 0; i < nprops; i++) {
    ...
    if (prop == NULL) {
      fprintf (stdout, "Warning:  Can't find property '%s' in element '%s'\n",
...
)
{
  ...
  elem = find_element (plyfile, elem_name);
  if (elem == NULL) {
    fprintf (stdout, "ply_get_other_properties: Can't find element '%s'\n",
             elem_name);
    return (NULL);
  }
...
void ascii_get_element(PlyFile *plyfile, char *elem_ptr)
{
  ...
  words = get_words (plyfile->fp, &nwords, &orig_line);
  if (words == NULL) {
    fprintf (stdout, "ply_get_element: unexpected end of file\n");
    exit (-1);
  }
...
void write_scalar_type (FILE *fp, int code)
{
  ...
  if (code <= StartType || code >= EndType) {
    fprintf (stdout, "write_scalar_type: bad data code = %d\n", code);
    exit (-1);
  }
...
double get_item_value(char *item, int type)
{
  ...
  switch (type) {
    ...
    case Float64:
      ...
      return (double_value);
    default:
      fprintf (stdout, "get_item_value: bad type = %d\n", type);
      exit (-1);
  }
...
)
{
  ...
  switch (type) {
    ...
    case Float64:
      ...
      break;
    default:
      fprintf (stdout, "write_binary_item: bad type = %d\n", type);
      exit (-1);
  }
}
...
)
{
  switch (type) {
    ...
    case Float64:
      ...
      break;
    default:
      fprintf (stdout, "write_ascii_item: bad type = %d\n", type);
      exit (-1);
  }
}
  ...
  switch (type) {
    ...
    case Float64:
      ...
      *uint_val = (unsigned int)(*double_val);
      break;
    default:
      fprintf (stdout, "get_stored_item: bad type = %d\n", type);
      exit (-1);
  }
}
  ...
  switch (type) {
    ...
    case Float64:
      ...
      *uint_val = (unsigned int)(*double_val);
      break;
    default:
      fprintf (stdout, "get_binary_item: bad type = %d\n", type);
      exit (-1);
  }
}
...
)
{
  switch (type) {
    ...
    default:
      fprintf (stdout, "get_ascii_item: bad type = %d\n", type);
      exit (-1);
  }
}
  ...
  switch (type) {
    ...
    case Float64:
      ...
      *pdouble = double_val;
      break;
    default:
      fprintf (stdout, "store_item: bad type = %d\n", type);
      exit (-1);
  }
}
...
static char *my_alloc(int size, int lnum, char *fname)
{
  ...
  if (ptr == 0) {
    fprintf(stdout, "Memory allocation bombed on line %d in %s\n", lnum, fname);
  }
...
char *setup_element_read_ply (PlyFile *ply, int index, int *elem_count)
{
  ...
  if (index < 0 || index > ply->num_elem_types) {
    fprintf (stdout, "Warning:  No element with index %d\n", index);
    return (0);
  }
...
)
{
  ...
  prop_ptr = find_property (elem, prop->name, &index);
  if (prop_ptr == NULL) {
    //fprintf (stdout, "Note: don't have property '%s' in element '%s'\n", prop->name, elem->name);
    return;
  }
  prop_ptr->internal_type  = prop->internal_type;
  prop_ptr->offset         = prop->offset;
...
)
{
  ...
  elem = find_element (plyfile, elem_name);
  if (elem == NULL) {
    fprintf(stdout,"describe_element_ply: can't find element '%s'\n",elem_name);
    exit (-1);
  }
...
)
{
  ...
  if (elem == NULL) {
    fprintf(stdout, "describe_other_properties_ply: can't find element '%s'\n",
            other->name);
    return;
  }
...
PlyPropRules *init_rule_ply (PlyFile *ply, char *elem_name)
{
  ...
  if (elem == NULL) {
    fprintf (stdout, "init_rule_ply: Can't find element '%s'\n", elem_name);
    exit (-1);
  }
  ...
  for (list = ply->rule_list; list != NULL; list = list->next) {
    ...
    if (!found_prop) {
      fprintf (stdout, "Can't find property '%s' for rule '%s'\n",
...
void modify_rule_ply (PlyPropRules *rules, char *prop_name, int rule_type)
{
  ...
  fprintf (stdout, "modify_rule_ply: Can't find property '%s'\n", prop_name);
  exit (-1);
}
...
void *get_new_props_ply(PlyFile *ply)
{
  ...
  for (i = 0; i < elem->nprops; i++) {
    ...
    "get_new_props_ply: Error combining properties that should be the same.\n");
      ...
      default:
        fprintf (stdout, "get_new_props_ply: Bad rule = %d\n",
	         rules->rule_list[i]);
	exit (-1);
    }
